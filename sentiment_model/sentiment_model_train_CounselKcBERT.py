# -*- coding: utf-8 -*-
"""7category_CounselKcBERT.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/100ekb7RmQG1Oe05cjHDZM_Ow93m4SrP8
"""
...
# 데이터셋 3개 불러오기
data1 = pd.read_excel("/content/drive/My Drive/Colab Notebooks/감정_문장별.xlsx")
data2 = pd.read_excel("/content/drive/My Drive/Colab Notebooks/정신건강_7분류.xlsx");
data3 =pd.read_excel("/content/drive/My Drive/Colab Notebooks/웰니스대화_7분류.xlsx");
#data2.dropna(axis = 0, inplace = True)

data1 = data1[['Sentence','Emotion']]
data1 = data1[data1.Emotion.notnull()]
# data1 = data1[data1['Emotion'] != '놀람']
# data1 = data1[data1['Emotion'] != '슬픔']
# data1 = data1[data1['Emotion'] != '혐오']
...
def preprocess_text(text):
  ...
  # new_text = repeat_normalize((new_text), num_repeats=2)
  return new_text


print(repeat_normalize('엄마가 키크라고 어렸을때부터 많이 먹였는데 크지안아요ㅠㅠㅠ'))
print(preprocess_text('엄마가 키크라고 어렸을때부터 많이 먹였는데 크지안아요ㅠㅠㅠ'))

data['Sentence'] = data['Sentence'].astype(str).apply(preprocess_text)

train, test = train_test_split(data,random_state=2000, test_size = 0.1)
...
#KoBert 토크나이저로 문장 -> 토큰
AutoTokenizer.from_pretrained("beomi/kcbert-base")
...
model = BertForSequenceClassification.from_pretrained("/content/drive/My Drive/CounselKcBERT2", num_labels=7)
model.cuda()

# 옵티마이저(loss function 값 최소화하도록하는 모델 인자를 찾는 것) 설정
# backpropogation으로 계산한 gradient로 실제 업데이트 하는 방식- optimizer
...
# 학습률을 조금씩 감소시키는 스케줄러 생성 (가변적으로 learning rate를 조정.)
...
logits = test_sentences(['6학년에서 중학교 올라갈 즈음에 인천으로 이사왔어요. '])
print(logits)
print(np.argmax(logits))
print(label_val[np.argmax(logits)])
...
logits = test_sentences(["너가 있어서 정말 좋았어"])
print(logits)
print(label_val[np.argmax(logits)])

logits = test_sentences(["걔네들이 나를 먼저 괴롭혔어"])
print(logits)
print(label_val[np.argmax(logits)])

logits = test_sentences(["난 요새 즐거워서 계속 웃음이 나와"])
['우울' '중립' '불안' '공포' '분노' '자살' '행복']
print(logits)
print(label_val[np.argmax(logits)])

logits = test_sentences([" 그 후 계속 은따."])
...
test_eval = test['Emotion']
test_predict = []
for i in range(test.shape[0]):
  test_predict.append(label_val[np.argmax(test_sentences([test.iloc[i].Sentence]))])

print(classification_report(test_eval, test_predict))